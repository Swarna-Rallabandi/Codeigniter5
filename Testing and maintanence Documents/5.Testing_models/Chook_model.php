<?php 
//main model for chook_data 
class Chook_model extends CI_Model{
    //method for shed_53 antenna_report
     //counting data for antenna ,enters and leaves on particualr date
 public function show_antenna_data_by_date($timestamp) {   
    
     //changing timestamp to date format
...
//$sql="SELECT antenna_name,tag_event  FROM shed_53 WHERE $condition GROUP  BY antenna_name,tag_event";  
     //sql querry for summing enters and leaves on particualr date with antenna names
...
sum (case when tag_event = 'Enter' then 1 else 0 end) as enters FROM shed_53 WHERE $condition GROUP BY antenna_name order by 2 desc, 3" ;     
     ...
     //sending query to data base to extract data and hold it in variable
$query = $this->db->query($sum);
     //checking if its not empty return restult otherwise it should return false
if ($query->num_rows() > 0) {
return $query->result();
} else {
return false;
     ...
     //method for shed_53 reader_report
    //counting data for reader, enters and leaves on particualr date
    public function show_reader_data_by_date($timestamp) {
      //changing timestamp to date format
$condition = "date(time_stamp)=" . "'" . $timestamp . "'";     
// $condition=date($timestamp);
//$sql="SELECT antenna_name,tag_event  FROM shed_53 WHERE $condition GROUP  BY antenna_name,tag_event"; 
        //sql querry for summing enters and leaves on particualr date with reader names
...
return false;
     ...
     //method for shed_53 epc_report
     //counting data for epc, enters and leaves
    public function show_epc_data() {
        
     //sql querry for summing enters and leaves in epc which is top 20 in data base
